...
- `JestSubscription.intro_offer` (`JestSubscriptionIntroOffer`, nullable) — carries
  `price` and `duration_periods` when the subscription has an introductory price
  and the wallet has never subscribed to it. The wallet pays `price` for the
  first `duration_periods` billing periods, then the standard `price` applies
  automatically.

...
- `JestSDK.navigation.redirect_to_team_game(game_slug, entry_payload = {}, skip_exit_confirm = false)` —
  redirects the player to another game owned by the same team as the calling
  game. Must be awaited; returns a `JestResult` with `error` set to
  `"game_not_found"`, `"different_team"`, or `"internal_error"` on failure.

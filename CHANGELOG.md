...
- `JestLoginMessageOptions.target_game_slug` — logs the user into a specific
  public game via the `{loginLink}` instead of the onboarding's flagship game.
  `JestInternal.reserve_login_message()` can now resolve with an
  `"invalid_target_game"` error when the slug doesn't resolve to a public game.

...
- `JestSDK.mark_first_milestone()` — reports that the player reached your
  game's first meaningful milestone (e.g. completing the tutorial or Level 1).
  Call it every time the milestone is reached; calls after the first in a
  session are no-ops. Implementing this event is required for launch.
